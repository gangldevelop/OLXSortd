      ...
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
        <div className="glass-panel p-6 w-full max-w-md mx-4">
          ...
          <div className="w-full bg-slate-200 rounded-full h-3 mb-4">
            <div className="h-3 rounded-full bg-blue-500 animate-pulse" style={{ width: '30%' }} />
          </div>
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="glass-panel p-6 w-full max-w-md mx-4 shadow-xl">
        ...
        <div className="flex justify-between items-center text-sm font-medium mb-4">
          <span className="text-slate-800">{Math.round(progress.progress)}%</span>
          {progress.estimatedTimeRemaining && progress.estimatedTimeRemaining > 0 && (
            <span className="text-slate-600">~{formatTime(progress.estimatedTimeRemaining)} remaining</span>
          )}
        </div>
        ...
        {progress.totalItems > 0 && (
          <div className="bg-slate-50 rounded-lg border border-slate-200 p-3 mb-4">
            <div className="text-xs font-semibold text-slate-600 mb-2">Processing Status</div>
